...
Model that includes FORESTAGE is a bit worse and FORESTAGE is not predicting anything
...
msal.age <- map(alist( SALAMAN ~ dpois(lambda),
                ...
                method="Nelder-Mead",
             data=salamanders)
precis(msal.age,corr = TRUE) #note : this can be really variable!
plot(precis(msal.age))  

(compare.out <- compare(msal1,msal.add,msal.age)) #model with FORESTAGE is a bit worse
plot(compare.out)
```
