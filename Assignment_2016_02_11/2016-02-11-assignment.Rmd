...
Pr(A|twins) = 0.1 * 0.5 / sum(0.1*0.5, 0.2*0.5) = `r  0.1 * 0.5 / sum(0.1*0.5, 0.2*0.5) `

Pr(B|twins) = 1 - Pr(A|twins) = `r 2/3`

So now we can sum the probability of twins if A or B

Pr(twins2) = Pr(A,twins) + Pr(B,twins) = 0.1*1/3 + 0.2*2/3 = `r 0.1*1/3 + 0.2*2/3`
...
Pr(A|no_twins) = 0.9 * 1/3 / sum(0.9*1/3, 0.8*2/3) = `r  0.9 * 1/3 / sum(0.9*1/3, 0.8*2/3)`
...
Pr(A|test) = 0.8 * 0.5 / sum(.8*.5, .35 * .5) = `r 0.8 * 0.5 / sum(.8*.5, .35 * .5)`

using our prior info on the likelihood of A and B from 2H3

Pr(A|test) = 0.8 * 0.36 / sum(.8*.36, .35 * .64) = `r 0.8 * 0.36 / sum(.8*.36, .35 * .64)`
